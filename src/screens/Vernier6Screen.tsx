...
type Props = StackScreenProps<RootStackParamList, 'Vernier6Screen'>;

interface Equipos {
    ID: string;
}

const Vernier6Screen: React.FC<Props> = ({ route, navigation }) => {
    ...
    useEffect(() => {
        const fetchEquipos = async () => {
            setLoading(true);
            try {
                const response = await axios.get('http://192.168.16.192:3002/api/calibraciones/vernier');
    ...
    const handleCalibrar = async () => {
        ...
        try {
            const response = await axios.get(`http://192.168.16.192:3002/api/calibraciones/vernier/${selectedEquipo.ID}`);
                    ...
                    Alert.alert(
                        ...
                        [
                            { text: 'OK', style: 'cancel' },
                            {
                                ...
                                onPress: () => {
                                    navigation.navigate('CalibrarV6Screen', {
    ...
    return (
        ...
        >
            <View style={styles.topContainer}>
                ...
                <Text style={styles.title}>Vernier</Text>
